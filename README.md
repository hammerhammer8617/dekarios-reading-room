...
一个移动端优先的 AI 共读书房与共同推理案件簿，运行在 ChatGPT Apps SDK + MCP 之上。用户可以导入自己的小说文本或漫画图片，记录自己和 AI 的阅读位置，也可以把小说、电子游戏或桌游中的线索整理成可协作的案件结构图。

本项目适合个人私有部署、学习和二次改造。公开仓库只提供代码、测试和原创 demo，不包含作者的阅读内容、聊天记录或线上数据。

## 当前状态

项目仍处于早期开发阶段，当前公开快照为 `v0.3.0`：

- 共读的默认输入改为 ChatGPT 官端读图：用户只需发送自己筛选出的书页截图或拍照。
- 每本书分别保存 Tav／Gale／共同思考、阅读进度、剧透边界、开放问题与 Notion 增量同步状态。
- 新增移动端书架、单本书记录页、「共读已开始」与「今天读到这里」卡片。
- 推理小说可按书维护人物、关系、线索、假说、时间线与下一次观察任务。
...
- 案件簿 MVP 已具备批量案情确认、人物／地点／物件节点、关系、猜想、拖拽结构图、情报委托与增量同步。
...
- iPad 移动端和沉浸阅读体验
- 小说、电子游戏与桌游案件簿
- 以单本书为单位的共读上下文、署名思考与开放问题
- 面向推理小说的书内人物关系、线索、假说、时间线和观察任务
- 《书名》｜书页边缘的 Notion 增量同步准备与确认状态
- 使用七张内置插画的移动端书架与「今天读到这里」卡片
- 书房首页内置「共同推理」入口，可在同一个组件中往返书房与案件簿
- Tav／Gale 分署名的线索、猜想、节点和关系
- 多条快速语法拆分、提交前逐条确认与原子批量保存
- 「盖尔的情报委托」下一次观察任务，可由 Tav 标记带回情报或暂时放下
- 只有确认收到后才推进的案件增量同步
...
- D1 保存 session、位置、偏好、source metadata，以及用户主动写入案件簿的案情、结构图和猜想；小说正文与漫画图片存放在私有 R2，本地 IndexedDB 仅作设备缓存。
