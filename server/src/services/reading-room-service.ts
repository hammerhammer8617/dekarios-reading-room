...
export class ReadingRoomService {
  ...
  async listBookshelf() {
    ...
    return this.sortSessions(database.sessions).map((session) => {
      ...
      const casebook = database.casebooks.find((item) => item.sessionId === session.id);
      return {
  ...
  async getBookContext(bookId: string): Promise<BookContext> {
    ...
    const casebook = database.casebooks.find((item) => item.sessionId === bookId);
    return {
  ...
  private ensureCasebook(database: ReadingDatabase, bookId: string, now: string) {
    let casebook = database.casebooks.find((item) => item.sessionId === bookId);
    if (!casebook) {
      casebook = this.emptyCasebook(bookId, now);
      database.casebooks.push(casebook);
    }
    return casebook;
  }

  private emptyCasebook(bookId: string, now: string): MysteryCasebook {
    return {
      sessionId: bookId,
      entities: [],
      relations: [],
  ...
  private findEntity(casebook: MysteryCasebook, name: string): CaseEntity | undefined {
  ...
  private requireEntity(casebook: MysteryCasebook, name: string) {
  ...
  private casebookSummary(casebook: MysteryCasebook) {
    return {
  ...
  private casebookItemCount(casebook: MysteryCasebook) {
    return (
